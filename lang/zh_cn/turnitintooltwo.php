...
$string['reportgenspeed_resubmission'] = '您已针对该作业提交论文，所提交论文的相似度报告已生成。如果您选择重新提交论文，那么之前提交的内容将被替换，并将生成新的报告。在 {$a->num_resubmissions} 次重新提交后，您需要在提交后等待 {$a->num_hours} 小时才能查看新报告。';$string['turnitinrepositoryoptions_help'] = '<strong>: </strong><br />教员可将 Turnitin 设定为添加文件至标准知识库、至机构的私有知识库、或不添加至知识库。<br /><br /><strong>: </strong><br />这一选项将允许教员查看作业设置，该作业设置允许学生通过 Turnitin 设定文件的储存位置。学生可以选择添加文件至标准学生知识库或添加至您机构的私有知识库。<br /><br /><strong>: </strong><br />所有文件都默认添加至标准学生知识库。<br /><br /><strong>: </strong><br />文件通过 Turnitin 将只用于供教员查看评分和初始查重。<br /><br /><strong>Submit all papers to the institutional repository: </strong><br />Turnitin 被设定为将所有论文储存至机构论文知识库。上传文件的查重工作将由您机构内的其他教员完成。';
$string['submitpapersto_help'] = '<strong>: </strong><br />Turnitin 被设定为不将上传文件储存至任何知识库。文件仅用于初始查重。<br /><br /><strong>: </strong><br />Turnitin 将只在标准知识库中储存上传文件的副本。选择此选项，Turnitin 对日后上传文件的查重工作将只使用已储存文件。<br /><br /><strong>: </strong><br />选择此选项，将 Turnitin 设定为只添加文件至您机构的私有知识库。上传文件的查重工作将只由您机构的其他教员完成。<br /><br />';
$string['migrationtoolv1list'] = '通过以下列表将版本 {$a} 之前迁移的 Moodle Direct V1 作业移除；这样可以解决 Moodle 成绩册中存在的误差。您也能识别尚未迁移到 V2 的 V1 作业。<br/><br/>若要了解更多信息，请参见版本说明：: <a href=http://guides.turnitin.com/releasenotes>guides.turnitin.com/releasenotes</a>';
